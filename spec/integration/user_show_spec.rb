...
RSpec.describe 'Test for the user show page', type: :system do
  describe 'Check the content for user show page' do
    before(:each) do
      ...
      @post_one = Post.create(author: @new_user, Title: 'Vue', Text: 'this is my first post', CommentsCounter: 0,
                              LikesCounter: 0)
      @post_two = Post.create(author: @new_user, Title: 'React', Text: 'I am gonna get a jon in may',
    ...
    it 'User should see the see the users recent posts' do
      expect(page).to have_content(@post_one.Text.to_s)
      expect(page).to have_content(@post_two.Text.to_s)
    end
